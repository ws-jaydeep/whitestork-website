...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      const cardTop = getElementOffset(card);
      const triggerStart = cardTop - stackPositionPx - itemStackDistance * i;
      // Consider an item active if scroll is at or past its triggerStart, with some margin
      if (scrollTop >= triggerStart - containerHeight * 0.3) {
    ...
    if (currentActiveIndex !== activeIndexRef.current) {
      ...
      onActiveItemChange?.(currentActiveIndex);
    }
